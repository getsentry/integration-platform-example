...
def convert_sentry_fields_to_dict(fields: Sequence[SentryField]) -> AlertRuleSettings:
    return reduce(lambda acc, field: {**acc, field.get("name"): field.get("value")}, fields, {})
...
def alert_rule_action() -> Response:
    ...
    if (
        not alert_rule_action_settings.get("title") or
        not alert_rule_action_settings.get("description")
    ):
        return jsonify({"message": "Title and description are required"}), 400

    if alert_rule_action_settings.get("userId"):
        user = User.query.filter(User.id == alert_rule_action_settings.get("userId")).first()
        if not user:
